...
from langchain_community.llms import OpenAI  # Keep if needed
...
class updated_data():
    ...
    def start_serverside(cls):
        ...
        while(True):
            # md_twitter, dummy_government_data, data (NewsAPI)
            dt_ = [None, None, None]
            dt_[0], dt_[1], dt_[2] = cls.get_data()

            # o_df, o_twit_df, output_recs
            outs_ = []
            outs_[0], outs_[1], outs_[2] = cls.prompt_ai(cls.__get_model, dt_[0], dt_[1], dt_[2])

            gnd_ = []
            gnd_[0], gnd_[1] = cls.gen_polygons(outs_[0], outs_[1], dt_[1], outs_[2])

            cls.set_poly(gnd_[0])
            cls.set_ai_rec(gnd_[1])
    ...
    @classmethod
    def prompt_ai(model, md_twitter, dummy_government_addys, data):
        """
    ...
    @classmethod
    def gen_polygons(o_df, o_twit_df, dummy_government_addys, output_rec):
        """
...
# # Initialize server side and begin running
# zones = updated_data()
# zones.start_serverside()

# @app.on_event("startup")
# async def startup_event():
#     global zones
#     zones = updated_data()
#     zones.start_serverside()  # Start only when FastAPI starts
